...
fn verify_internally<'a>(verify_key: &VerifyKeyFormatInt, data_with_sig: &'a [u8], sign_head: &SignHead) -> Result<&'a [u8], Error>
{
	//check if the verify key is the right key id
	if verify_key.key_id != sign_head.id {
		return Err(Error::SigFoundNotKey);
	}

	//verify the data with the right key
	let (encrypted_data_without_sig, check) = verify(&verify_key.key, data_with_sig)?;
...
fn encrypt_raw_asymmetric_internally(
	reply_public_key: &PublicKeyFormatInt,
	data: &[u8],
	sign_key: Option<&SignKeyFormatInt>,
) -> Result<(EncryptedHead, Vec<u8>), Error>
{
	let mut encrypt_head = EncryptedHead {
		id: reply_public_key.key_id.to_string(),
		sign: None,
	};

	let mut encrypted = encrypt_asymmetric(&reply_public_key.key, data)?;

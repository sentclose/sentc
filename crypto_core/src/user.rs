...
{
	//info about the raw master key (not encrypted by the password!)
...
Every user got a master key, this master key encrypts every other user key.
...
Only this key gets sent to the server, not the password.

A public / private encryption key pair and a sign / verify key pair get created too.

Both private and sign keys get encrypted by the master key.

<br>

## For key update:
 register the user again with a password, it should be the same password for usability, but it can be a different password too.
Then login again and encrypt everything that was encrypted by the old keys with the new keys
(e.g., group keys (encrypted by a public key), or direct encrypted data).
*/
...
) -> Result<RegisterOutPut<St::PublicKey, Sign::VerifyKey, H::CRV, H::HAK>, Error>
{
	...
	//2. create a static pub/pri key pair for encryption and decryption
	let (sk, public_key) = St::generate_static_keypair()?;

	//3. create a sign key pair for sign and verify
...
The server generates the salt when the user first interacts with the server.
*/
...
to create the new client random value, the new encrypted master key, and the new hashed auth-key
...
) -> Result<ChangePasswordOutput<H::CRV, H::HAK, H::DAK>, Error>
{
	...
	//the 2nd check is necessary because a master key from different alg can have different length
	let (client_random_value, hashed_authentication_key_bytes, encrypted_master_key, encrypted_master_key_alg) =
...
/**
Creates a safety number in byte of a given verified key and additional user information like the user id or username.

...
use crate::core::sym::{raw_decrypt as aes_decrypt, raw_encrypt as aes_encrypt, Aes256GcmKey, FIPS_OPENSSL_AES_GCM};
...
pub const FIPS_OPENSSL_PW_HASH_ALG: &str = "fips_openssl_pbkdf2_hmac";

pub struct ClientRandomValue([u8; 16]);

crypto_alg_str_impl!(ClientRandomValue, FIPS_OPENSSL_PW_HASH_ALG);
...
impl PwHash for PwHasherGetter
{
	...
	{
		if let Some(alg) = alg {
			if alg != FIPS_OPENSSL_PW_HASH_ALG {
...
) -> Result<(ClientRandomValue, HashedAuthenticationKey, Vec<u8>, &'static str), Error>
{
	...
	let encrypted_master_key = aes_encrypt(&derived_encryption_key_bytes, master_key.as_ref())?;
...
mod test
{
	...
	fn test_derived_keys_from_password()
	{
		...
		assert_eq!(client_random_value.get_alg_str(), FIPS_OPENSSL_PW_HASH_ALG);
	...
	fn test_password_to_encrypt_and_decrypt()
	{
		...
		let (salt, aes_key_for_encrypt) = PwHasherGetter::password_to_encrypt(b"my fancy password").unwrap();
		...
		let aes_key_for_decrypt = PwHasherGetter::password_to_decrypt(b"my fancy password", &salt).unwrap();

...
use crate::keys::{SignKey, SymmetricKey, VerifyKey};
...
impl SymmetricKey
{
	...
	fn finish_raw_encrypt(&self, encrypted: Vec<u8>, sign_key: Option<&SignKey>) -> Result<(EncryptedHead, Vec<u8>), SdkUtilError>
	{
		match sign_key {
			None => {
				Ok((
	...
	pub fn encrypt_raw(&self, data: &[u8], sign_key: Option<&SignKey>) -> Result<(EncryptedHead, Vec<u8>), SdkUtilError>
	{
	...
	pub fn encrypt_raw_with_aad(&self, data: &[u8], aad: &[u8], sign_key: Option<&SignKey>) -> Result<(EncryptedHead, Vec<u8>), SdkUtilError>
	{
	...
	pub fn encrypt(&self, data: &[u8], sign_key: Option<&SignKey>) -> Result<Vec<u8>, SdkUtilError>
	{
	...
	pub fn encrypt_with_aad(&self, data: &[u8], aad: &[u8], sign_key: Option<&SignKey>) -> Result<Vec<u8>, SdkUtilError>
	{
	...
	pub fn encrypt_string(&self, data: &str, sign_key: Option<&SignKey>) -> Result<String, SdkUtilError>
	{
	...
	pub fn encrypt_string_with_aad(&self, data: &str, aad: &str, sign_key: Option<&SignKey>) -> Result<String, SdkUtilError>
	{
